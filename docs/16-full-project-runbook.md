...
```bash
ssh ssafy@192.168.110.142 \
  'CONVEYOR_MODE=stepper CONVEYOR_STEP_PIN=27 CONVEYOR_DIR_PIN=17 CONVEYOR_ENABLE_PIN=22 CONVEYOR_ENABLE_ACTIVE_HIGH=0 CONVEYOR_DIR_ACTIVE_HIGH=0 CONVEYOR_GPIO_BACKEND=gpiod CONVEYOR_STEPS=200 python3 ~/smart-assembly-transport-edge/conveyor_control.py start'
```
...
```bash
ssh ssafy@192.168.110.142 \
  'CONVEYOR_MODE=stepper CONVEYOR_STEP_PIN=27 CONVEYOR_DIR_PIN=17 CONVEYOR_ENABLE_PIN=22 CONVEYOR_ENABLE_ACTIVE_HIGH=0 CONVEYOR_DIR_ACTIVE_HIGH=0 CONVEYOR_GPIO_BACKEND=gpiod CONVEYOR_DIRECTION=reverse CONVEYOR_STEPS=200 python3 ~/smart-assembly-transport-edge/conveyor_control.py start'
```
...
```bash
ssh ssafy@192.168.110.142 \
  'python3 ~/smart-assembly-transport-edge/conveyor_control.py start'
```

```bash
ssh ssafy@192.168.110.142 \
  'python3 ~/smart-assembly-transport-edge/conveyor_control.py stop'
```
...
```bash
ssh ssafy@192.168.110.142 \
  'python3 ~/smart-assembly-transport-edge/conveyor_control.py emergency-stop'
```

> 주의: `scripts/edge/conveyor_control.py` 안의 GPIO/PWM TODO는 실제 모터 드라이버 배선에 맞게 수정되어 있어야 진짜 모터가 동작합니다.
